Rollbar.configure do |config|
  ...
  config.access_token = Sidekiq.server? ? ENV['ROLLBAR_SIDEKIQ_TOKEN'] : ENV['ROLLBAR_TOKEN']

  # Here we'll disable in 'test' and 'development':
  enabled = !(Rails.env.test? || Rails.env.development?)
  config.js_enabled = config.enabled = enabled
...
module Rollbar
  def self.raise_or_notify(e, opts = {})
    ...
    opts[:use_exception_level_filters] = true unless opts.key?(:use_exception_level_filters)
