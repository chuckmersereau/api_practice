...
RSpec.describe MailChimp::ExportContactsWorker do
  let(:mail_chimp_account) { build(:mail_chimp_account, primary_list_id: 'list_one') }
  ...
  it 'starts the export if the list used is not the primary list' do
    ...
    MailChimp::ExportContactsWorker.new.perform(mail_chimp_account, 'list_two', [1])
  end
  ...
  it 'does not start the export if the list used is the primary list' do
    ...
    MailChimp::ExportContactsWorker.new.perform(mail_chimp_account, 'list_one', [1])
  end
end
