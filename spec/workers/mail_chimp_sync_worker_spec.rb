...
describe MailChimpSyncWorker, '#perform' do
  it 'subscribes users signed in recently' do
    user = create(:person_with_email, email: 'john@example.com', sign_in_count: 1, current_sign_in_at: 1.day.ago)
    create_member =
  ...
  it 'unsubscribes users signed in a while ago' do
    user = create(:person_with_email, email: 'john@example.com', sign_in_count: 1, subscribed_to_updates: true,
