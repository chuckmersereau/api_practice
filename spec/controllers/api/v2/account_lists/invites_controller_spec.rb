...
describe Api::V2::AccountLists::InvitesController, type: :controller do
  ...
  let!(:invite) { create(:account_list_invite, account_list: account_list, invited_by_user: user) }
  let!(:second_invite) { create(:account_list_invite, account_list: account_list, invited_by_user: user) }
  let(:id) { invite.uuid }
  ...
  describe '#update / accept' do
    ...
    it 'returns a 401 when a user dis not logged in' do
      api_logout
      expect do
