...
RSpec.describe ApplicationPreloader::IncludeAssociationsFetcher do
  context '#fetch_include_associations' do
    ...
    it 'includes all included resources and the appropriate associations for each of those' do
      ...
      expect(Api::V2::Contacts::PeoplePreloader).to receive(:new).with(['email_addresses'], field_params, 'people').and_call_original
      expect_any_instance_of(Api::V2::Contacts::PeoplePreloader).to receive(:associations_to_preload).and_return([:email_addresses])

      expect(subject.fetch_include_associations(include_params, field_params)).to match_array([:contacts_that_referred_me, people: [:email_addresses]])
    end
  end
end
