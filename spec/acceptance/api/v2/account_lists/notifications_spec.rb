  ...
  context 'authorized user' do
    ...
    get '/api/v2/account_lists/:account_list_id/notifications' do
      ...
      example_request 'list notifications of account list' do
        ...
        expect(resource_object.keys).to eq %w(contact-id notification-type-id event-date cleared donation-id)
        expect(status).to eq 200
      end
    end
    get '/api/v2/account_lists/:account_list_id/notifications/:id' do
      with_options scope: [:data, :attributes] do
        response_field 'event-date',              'Event Date', 'Type' => 'Date'
        response_field 'contact-id',              'Contact ID', 'Type' => 'Integer'
        response_field 'notification-type-id',    'Notification Type ID', 'Type' => 'Integer'
        response_field :cleared,                  'Cleared', 'Type' => 'Boolean'
        response_field 'donation-id',             'Donation ID', 'Type' => 'Integer'
      end
