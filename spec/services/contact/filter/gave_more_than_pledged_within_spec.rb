...
RSpec.describe Contact::Filter::GaveMoreThanPledgedWithin do
  ...
  describe '#query' do
    let!(:first_contact) { create(:contact, account_list: account_list, donor_accounts: [build(:donor_account)]) }
    let!(:second_contact) { create(:contact, account_list: account_list, donor_accounts: [build(:donor_account, account_number: '123')], pledge_amount: 20.00) }
    ...
    let!(:first_donation_from_second_contact) do
      create(:donation, donation_date: 2.months.ago, designation_account: account_list.designation_accounts.first, donor_account: second_contact.donor_accounts.first, amount: 50.00)
    end
    ...
    context 'contacts that have started giving within date range' do
      it 'returns the correct contacts' do
        expect(described_class.query(contacts, { gave_more_than_pledged_within: Range.new(3.months.ago, 1.month.ago) }, [account_list])).to match_array([second_contact])
      end
    end
  end
end
