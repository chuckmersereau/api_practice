  ...
  describe '#query' do
    ...
    context 'filter with updated_at value' do
      it 'returns only tasks that have the updated_at value' do
        expect(described_class.query(tasks, { updated_at: task_one.updated_at }, nil).to_a).to match_array [task_one]
      end
    end
    ...
    context 'filter with updated_at range' do
      ...
      it 'returns only tasks that are within the updated_at range' do
        expect(described_class.query(tasks, { updated_at: one_day_ago }, nil).to_a).to match_array [task_one]
