...
RSpec.describe Task::Filter::ContactInfoWorkPhone do
  ...
  describe '#query' do
    ...
    context 'no filter params' do
      it 'returns nil' do
        ...
        expect(described_class.query(tasks, { contact_info_work_phone: {} }, account_list)).to eq(nil)
        expect(described_class.query(tasks, { contact_info_work_phone: [] }, account_list)).to eq(nil)
        expect(described_class.query(tasks, { contact_info_work_phone: '' }, account_list)).to eq(nil)
      end
    end
    ...
    context 'filter by no mobile phone' do
      it 'returns only tasks with contacts that have no mobile home phone' do
        expect(described_class.query(tasks, { contact_info_work_phone: 'No' }, account_list).to_a).to match_array [task_two, task_three, task_four]
      end
    end

    context 'filter by mobile phone' do
      it 'returns only tasks with contacts that have a home phone' do
        expect(described_class.query(tasks, { contact_info_work_phone: 'Yes' }, account_list).to_a).to match_array [task_one]
      end
    end
  end
end
