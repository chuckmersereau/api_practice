...
class AccountList < ActiveRecord::Base
  ...
  def mc_subscribe_users(group)
    ...
    users.each do |u|
      ...
      vars = { EMAIL: u.email.email, FNAME: u.first_name, LNAME: u.last_name,
               GROUPINGS: [{ id: ENV['MAILCHIMP_GROUPING_ID'], groups: group }] }
  ...
  def mc_unsubscribe_users(group)
    ...
    users.each do |u|
      ...
      result = gb.list_member_info(id: ENV['MAILCHIMP_LIST'], email_address: [u.email.email])
      next unless result['success'] > 0
      result['data'].each do |row|
        next unless row['email'] && row['merges']
        grouping = row['merges']['GROUPINGS'].detect { |g| g['id'] == ENV['MAILCHIMP_GROUPING_ID'] }
        next unless grouping
        groups = grouping['groups'].split(', ')
        groups -= [group]
        vars = { GROUPINGS: [{ id: ENV['MAILCHIMP_GROUPING_ID'], groups: groups.join(', ') }] }
        gb.list_update_member(id: ENV['MAILCHIMP_LIST'], email_address: row['email'], merge_vars: vars,
