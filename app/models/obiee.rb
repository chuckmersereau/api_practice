...
class Obiee
  ...
  def initialize
    ...
    client = get_client ENV['OBIEE_BASE_URL'] + 'nQSessionService'
  ...
  def report_sql(session_id, path, vars = {})
    get_report_client = get_client ENV['OBIEE_BASE_URL'] + 'reportService'
  ...
  def report_results(session_id, report_sql)
    run_report_client = get_client ENV['OBIEE_BASE_URL'] + 'xmlViewService'
