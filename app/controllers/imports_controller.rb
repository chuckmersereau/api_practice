class ImportsController < ApplicationController
  def create
    if import_params
      ...
      if import.save
        flash[:notice] = _('MPDX is currently importing your contacts from %{source}. You will receive an email when the import is complete.').localize % { source: params[:import][:source].gsub('_', ' ') }
      else
